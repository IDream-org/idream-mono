...
const CategoryItemsPageGallary = () => {
  ...
  const { data: category } = useGetCategoryQuery({ collectionId, categoryId });

  const userActions = new UserActions(session, collection);

  const [uploadImage] = useUploadImageMutation();
  const [addPhoto] = useAddCategoryPhotoMutation();
  const [removePhoto] = useRemoveCategoryPhotoMutation();
  ...
  const handleRemovePhoto = async (photoId: string) => {
    try {
      await removePhoto({ collectionId, categoryId, photoId }).unwrap();
...
export default CategoryItemsPageGallary;
